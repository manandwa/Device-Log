...
let MTA_Test_Status = MTA_Remaining_Tested.toString();
let MTA_Built_Status = MTA_Remaining.toString();
if((MTA_Remaining == 0) && (MTA_Remaining_Tested == 0)) {
  document.getElementById("MTA_Status").innerHTML = MTA_Total + " " + "MTAs Built and Tested";
} else {
  document.getElementById("MTA_Status").innerHTML = "Need to build " + " " + MTA_Built_Status + " MTAs" + " and Need to test " + MTA_Test_Status + " MTAs";
}
